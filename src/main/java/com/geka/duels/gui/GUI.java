...
import com.geka.duels.utilities.inventory.ItemBuilder;
...
public class GUI<T> {
    ...
    public void update(List<T> entries) {
        ...
        if (!entries.isEmpty()) {
            ItemStack next = ItemBuilder.builder().type(Material.PAPER).name("&aPagina siguiente").build();
            ItemStack previous = ItemBuilder.builder().type(Material.PAPER).name("&aPagina anterior").build();
            ItemStack separator = ItemBuilder.builder().type(Material.STAINED_GLASS_PANE).name(" mc.doricrafters.net ").build();
            ...
            Inventory current = Bukkit.createInventory(null, 54, title + " (pag. 1/1)");
            ItemStack defaultDisplayed;
